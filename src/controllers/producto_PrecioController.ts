...
import {
  addProducto_Precio,
  deleteProducto_Precio,
  getAllProducto_Precio,
  getProducto_PrecioById,
  getProducto_PrecioByProductId,
  updateProducto_Precio,
} from "../services/producto_PrecioServices";
  ...
  try {
    ...
    if (!producto || !precio) {
      res
        ...
        .json({
          message:
            "Deve proporcionar un producto:{id_producto: number} y un precio: decimal",
        });
      return;
    }
...
export const updateProducto_PrecioController = async (
  ...
  try {
    ...
    const updatedProducto_Precio = await updateProducto_Precio(
      id,
      fieldsToUpdate
    );
...
export const getProducto_PrecioByIdController = async (
  ...
  try {
    ...
    if (producto_Precio) {
      res
        .status(200)
        .json({ ...producto_Precio, precio: Number(producto_Precio.precio) });
    } else {
  ...
  try {
    ...
    if (producto_Precio) {
      ...
      res
        ...
        .json({
          message:
            "No se encontraron producto_precio para el ID de producto proporcionado",
        });
    }
  } catch (error) {
