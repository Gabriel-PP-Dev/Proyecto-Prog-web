...
const getProductosByTiendaSortedByQuantityController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!id) {
        res
            .status(400)
            .json({
            message: "Asegúrese de pasar como información: id (parámetro, id de la tienda de la que desea obtener los productos)",
        });
        return;
    }
    ...
    try {
        ...
        if (!id || !id_tienda_origen || !id_tienda_destino || !cantidad) {
            res
                .status(400)
                .json({
                message: "Asegúrese de pasar como información: id (parámetro), id_tienda_origen, id_tienda_destino y cantidad (body)",
            });
            return;
        }
        // Validar si el id es un uuid válido
        ...
        // Validar si el id es un uuid válido
        if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(id_tienda_destino)) {
            res.status(400).json({ message: "El id debe ser un uuid válido" });
            return;
        } // Validar si el id es un uuid válido
        if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(id_tienda_origen)) {
            res.status(400).json({ message: "El id debe ser un uuid válido" });
            return;
        }
...
const getProductoByIdController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!id) {
        res
            .status(400)
            .json({ message: "Asegúrese de pasar como información: id (parámetro)" });
        return;
    }
    // Validar si el id es un uuid válido
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(id)) {
...
const updateProductoController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!nombre) {
        res
            .status(400)
            .json({
            message: "Asegúrese de pasar como información: nombre por el body",
        });
        return;
    }
...
const getProductosByNameController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!name) {
        res
            .status(400)
            .json({
            message: "Asegúrese de pasar como información: name (parámetro)",
        });
        return;
    }
    try {
